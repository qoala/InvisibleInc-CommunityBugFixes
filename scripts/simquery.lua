...
function simquery.canSoftPath( sim, unit, startcell, endcell, ... )
	...
	for i,cellUnit in ipairs(endcell.units) do
		if unit and (unit:getTraits().dynamicImpass or cellUnit:getTraits().emitterID) and cellUnit:getTraits().dynamicImpass then
			...
			elseif (startcell.x == x0 and startcell.y == y0 and sim:canUnitSee( unit, endcell.x, endcell.y )) or (math.abs(x0 - endcell.x) + math.abs(y0 - endcell.y) == 1 and unit:getTraits().rememberedImpass[endcell.id]) then
