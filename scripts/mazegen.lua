...
function mazegen.saveMazePNG(cxt, filename)
        ...
        if type(room) == "table" then
            for j, tile in pairs(room) do
                if tile.x and tile.y then
                    ...
                    x0, y0 = (tile.x - 1) * CELL_SIZE + WALL_WIDTH,
                             (tile.y - 1) * CELL_SIZE + WALL_WIDTH
                    ...
                    for k = 0, 6, 2 do
                        if tile.sides and tile.sides[k] then
                            x0, y0, x1, y1 = tile.x * CELL_SIZE, tile.y * CELL_SIZE,
                            ...
                            elseif k == simdefs.DIR_W then
                                x0, y0, x1 = x0 - CELL_SIZE, y0 - CELL_SIZE,
                                             x1 - CELL_SIZE + WALL_WIDTH
                            elseif k == simdefs.DIR_S then
                                x0, y0, y1 = x0 - CELL_SIZE, y0 - CELL_SIZE,
                                             y1 - CELL_SIZE + WALL_WIDTH
                            end

                            if tile.sides[k].door and
                                    (tile.sides[k].locked or tile.sides[k].guarddoor) then
    ...
    for i, unit in pairs(cxt.units) do
        ...
        elseif unit.template == "lab_safe" or unit.template == "lab_safe_tier2" or unit.template ==
                "guard_locker" or unit.template == "vault_safe_1" or unit.template == "vault_safe_2" or
                unit.template == "vault_safe_3" or unit.template == "power_core" then
            image:fillRect(w - x0, y0, w - x1, y1, 0, 1, 0, 1)
        elseif unit.template == "security_camera_1x1" or unit.template == "security_soundBug_1x1" or
                unit.template == "turret" or unit.template == "turret_generator" then
            image:fillRect(w - x0, y0, w - x1, y1, 1, 0, 0, 1)
        elseif unit.template == "detention_processor" or unit.template == "public_terminal" or
                unit.template == "vault_processor" or unit.template == "research_processor" or
                unit.template == "data_node" or unit.template == "transformer_terminal" or
                unit.template == "diagnostic_terminal" or unit.template ==
                "research_security_processor" or unit.template == "ending_jackin" or unit.template ==
                "yellow_level_console" then
            image:fillRect(w - x0, y0, w - x1, y1, 1, 1, 0, 1)
        elseif unit.template == "item_store" or unit.template == "item_store_large" or unit.template ==
                "augment_grafter" or unit.template == "augment_drill" or unit.template ==
                "server_terminal" or unit.template == "mini_server_terminal" then
