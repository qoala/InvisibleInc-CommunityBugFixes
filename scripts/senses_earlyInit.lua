...
function Senses:processInterestShared(sim, evData)
    ...
    if evData.target then
        ...
        if simquery.couldUnitSee(sim, self.unit, evData.target) -- new: instead of only raycast, also check if interest cell is in vision range.
        -- (simquery.couldUnitSeeCell does both)
        and simquery.couldUnitSeeCell(sim, self.unit, sim:getCell(evData.target:getLocation())) then
            self:addTarget(evData.target)
            return
        end
    end
    self:addInterest(
            evData.interest.x, evData.interest.y, simdefs.SENSE_RADIO,
...
function Senses:processSoundTrigger(sim, evData)
    ...
    if evData.ignoreSight == nil then
        ...
        if sim:canUnitSee(self.unit, evData.x, evData.y) and
                (self.unit:getTraits().seesHidden or
                        not simquery.checkCover(sim, self.unit, evData.x, evData.y)) then
            return
        end
    end
    ...
    self:addInterest(
            evData.x, evData.y, simdefs.SENSE_HEARING, simdefs.REASON_NOISE, evData.sourceUnit)
end
